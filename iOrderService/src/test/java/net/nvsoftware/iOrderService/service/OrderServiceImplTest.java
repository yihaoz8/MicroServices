...
class OrderServiceImplTest {
   ...
   @Test
   void testWhenOrderIdNotFount() {
      when(orderRepository.findById(anyLong())).thenReturn(Optional.ofNullable(null));

      RuntimeException runtimeException = Assertions.assertThrows(RuntimeException.class, () -> orderService.getOrderDetailByOrderId(2));
      Assertions.assertEquals("OrderService getOrderDetailByOrderId Not Found for: 2", runtimeException.getMessage());
   ...
   void testWhenPlaceOrderSuccess() {
      ...
      when(productServiceFeighClient.reduceQuantity(anyLong(), anyLong()))
      ...
      verify(productServiceFeighClient, times(1)).reduceQuantity(anyLong(), anyLong());
